...
const SELECT_COLUMNS: &str = "id, topic_id, content_type, bytes, uploaded_by, uploaded_at";
...
fn to_attachment(row: Row) -> Option<Attachment> {
...
impl AttachmentRepository for PgAttachmentRepository {
    ...
    async fn list_for(&self, topic_id: TopicId) -> Vec<Attachment> {
        sqlx::query_as::<_, Row>(&format!(
            "SELECT {SELECT_COLUMNS} FROM attachments \
             WHERE topic_id = $1 ORDER BY uploaded_at ASC"
        ))
    ...
    async fn count_for(&self, topic_id: TopicId) -> u64 {
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM attachments WHERE topic_id = $1")
            .bind(topic_id.as_uuid())
            .fetch_one(&self.pool)
            .await
