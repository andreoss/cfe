...
pub struct MySqlPollRepository {
    pool: MySqlPool,
}

impl MySqlPollRepository {
...
impl PollRepository for MySqlPollRepository {
    async fn save(&self, poll: &Poll) {
        sqlx::query("INSERT INTO polls (id, topic_id, question, created_at) VALUES (?, ?, ?, ?)")
